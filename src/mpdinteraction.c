 ...
 *  - change GtkToggleButton to GtkToolToggleButton because of maemo player
 */
...
int connect_to_mpd()
{
	...
	mpd_set_connection_timeout(connection, cfg_get_single_value_as_float_with_default(config,"connection","timeout",3.0));
...
void random_pl(GtkToggleToolButton *tb)
{
	if(gtk_toggle_tool_button_get_active(tb) != mpd_player_get_random(connection))
		mpd_player_set_random(connection, !mpd_player_get_random(connection));
}

void repeat_pl(GtkToggleToolButton *tb)
{
	if(gtk_toggle_tool_button_get_active(tb) != mpd_player_get_repeat(connection))
...
void connection_pref_construct(GtkWidget *container)
{
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(glade_xml_get_widget(connection_pref_xml, "timeout_spin")), 
			cfg_get_single_value_as_float_with_default(config,"connection", "timeout",3.0));
