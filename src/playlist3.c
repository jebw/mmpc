 ...
 *
 * Modifications by Holger Macht <holger@homac.de> on Sun, 11 Feb 2007:
 *  - comment out everything related to tb_pl2 button, I don't have that in maemo
 *  - add hildonized menu
 *
...
void pl3_initialize_tree()
{
	...
	GtkTreeSelection *sel = gtk_tree_view_get_selection(GTK_TREE_VIEW(cat_tree));

	gtk_widget_tap_and_hold_setup(GTK_WIDGET(cat_tree), NULL, NULL, 0);
...
void pl3_cat_row_activated(GtkTreeView *tree, GtkTreePath *tp, GtkTreeViewColumn *col)
{
	...
	{
		/* in maemo, rows are collapsed automatically when
		 * clicked. This is due to the missin expander arrow. So
		 * disable the manual expanding/collapsing */
...
/* this is called by tap_and_hold */
int pl3_cat_tree_button_release_event(GtkTreeView *tree, gpointer data)
{
	int i;
	gint type  = pl3_cat_get_selected_browser();
	int menu_items = 0;
	...
	menu_items	+= pl3_current_playlist_browser_cat_menu_popup(menu, type,tree,NULL);
	menu_items	+= pl3_file_browser_cat_popup(menu,type,tree,NULL);
	menu_items	+= pl3_artist_browser_cat_popup(menu, type, tree, NULL);
	for(i=0; i< num_plugins;i++)
	{
...
*/
	...
	mmpc_attach(HILDON_WINDOW(glade_xml_get_widget(pl3_xml, "pl3_win")),
		    G_CALLBACK(pl3_close), NULL);
}
